...
map f ls = foldRight (\l acc -> f l :. acc) Nil ls
...
filter f ls = foldRight includeOrNot Nil ls
  where includeOrNot l acc = if f l then l :. acc else acc
